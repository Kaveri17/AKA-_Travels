  ...
  {blog?.slice(0, 1).map(blogs => (
    ...
    <React.Fragment key={blogs.id}>
      ...
      <div className="  ps-4 pe-4 pb-16  mx-auto w-5/6">
        ...
        <Slider {...settings}>
          {gallery.map(gall => {
            return <div className='w-3/4 relative px-2'>

                {/* <div className='relative'> */}
                  <img src={`${API}/${gall.image}`} alt="" className='pic4 h-96 w-full ' />
                  <h1 className='text-xl pt-2'>{gall.image_title} <i className="fa-solid fa-arrow-right"></i></h1>
                  <p className='rounded-lg bg-cyan-300 absolute left-10 top-10 text-white'>
                    (2 Trips)
                  </p>
                </div>
